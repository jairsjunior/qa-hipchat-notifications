...
//   const DEFAULT_TARGET = 'https://api.hipchat.com';
   ...
   */
  function __construct($api_target, $chat_id) {
  ...
  public function curl_request($url, $post_data = null) {
    ...
    if (strlen($response) == 0) {
      $errno = curl_errno($ch);
      $error = curl_error($ch);
      throw new Telegram_Exception(self::STATUS_BAD_RESPONSE,
    ...
    if ($code != self::STATUS_OK) {
      throw new Telegram_Exception($code,
...
class Telegram_Exception extends \Exception {
  ...
  public function __construct($code, $info, $url) {
    $message = "Telegram API error: code=$code, info=$info, url=$url";
